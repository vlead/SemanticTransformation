...
* form
It will get webpage which user wants to annotate.
#+BEGIN_SRC python :tangle ../../code/selenium/app.py :eval no
...
* webdriver
Selenium driver is a web automation tool which I am using here to open webpage and inject javascript into into the DOM which will load Annolet.
Function run_selenium will first check the present browser and get its version, browser name, platform(windows/linux/mac) and user_agent string which is related to browser. This information will tell about the current browser.
Then object of webdriver will be created which will trigger new browser window and open the URL given by user. 
It will wait(for maximum 30 seconds) for header of page to load completely and then inject JS into it.
